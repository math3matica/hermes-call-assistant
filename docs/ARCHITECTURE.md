...
Hermes plugin adapter (public register(ctx))
  -> safe status tool + /call-assistant
  -> explicit `hermes call-assistant` terminal activation
...
| Status/diagnostic reporting | Call Assistant | A | Plugin tools |
| call assistant runtime state, bridge, protocol, post-call store | Call Assistant | A/B | Plugin package and profile-local artifacts |
...
| `hermes call-assistant` activation | call assistant plugin | A/B | Explicit CLI subprocess boundary |
...
| Learning handoff | Call Assistant | A/B | Plugin-owned durable proposal and host job boundary |
...
but inactive, or active through `hermes call-assistant`. The special runtime does
...
uses the same Call Assistant turn loop without audio.
...
project proves the call assistant runtime/control-plane boundary and adapter packaging,
...
`call_assistant_status`, `/call-assistant`, and the separate `call-assistant` CLI
...
specialized Call Assistant session. Call Assistant owns model conversation, allowlisted
...
independently verified. A missing or failed call assistant runtime is reported as a
call-assistant failure, while ordinary Hermes Voice Mode remains unaffected.
