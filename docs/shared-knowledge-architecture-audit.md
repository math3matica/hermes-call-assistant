...
| `call assistant runtime.note_search` | Searches authorized Markdown roots, bounded to 4 results/10,000 chars | A/H retrieval | Yes | Token/paragraph search; excludes `.rex-vault-backups` and `Voice Workspace`. Does not search raw transcripts. |
...
- `OBSIDIAN_VAULT_PATH` is required to start Call Assistant.
- `VaultAdapter` delegates durable document mutations to the existing configured document store provider.
- `RexVoiceWorkspace` authorizes the Vault-local `Voice Workspace` plus explicitly granted read/search roots from `HERMES_CALL_ASSISTANT_PREPARED_ROOTS`.
